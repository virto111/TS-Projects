...
function validate(object) {
    const keys = Reflect.getMetadata("validation", object);
    let errorMap = {};
    if (!keys || !Array.isArray(keys)) {
        return errorMap;
    }
    for (const key of keys) {
        const rules = Reflect.getMetadata("validation", object, key);
...
function addValidation(target, propertyKey, validator, validationOptions) {
    ...
    let objectProperties = Reflect.getMetadata("validation", target) || [];
    if (!objectProperties.includes(propertyKey)) {
        objectProperties.push(propertyKey);
        Reflect.defineMetadata("validation", objectProperties, target);
    }
    // Make sure we capture validation rule
    let validators = Reflect.getMetadata("validation", target, propertyKey) || [];
    let validationRule = {
        validator: validator,
        validationOptions: validationOptions
    };
    validators.push(validationRule);
    Reflect.defineMetadata("validation", validators, target, propertyKey);
}
